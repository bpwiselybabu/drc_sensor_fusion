...
void testStereoRefinementEval()
{
	...
	string dir=string("dataset/Middlebury/");
	string sequence=string("teddy");
	...
	sprintf(name,"%s%s/%s.png",dir.c_str(),sequence.c_str(),"groundtruth");
	Mat gt = imread(name,0);

	sprintf(name,"%s%s/%s.png",dir.c_str(),sequence.c_str(),"all");
	std::cout<<name<<std::endl;
	Mat all=imread(name,0);
	sprintf(name,"%s%s/%s.png",dir.c_str(),sequence.c_str(),"disc");
	Mat disc=imread(name,0);
	sprintf(name,"%s%s/%s.png",dir.c_str(),sequence.c_str(),"nonocc");
