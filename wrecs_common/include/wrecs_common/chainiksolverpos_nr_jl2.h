...
#ifndef KDLCHAINIKSOLVERPOS_NR_JL2_H
#define KDLCHAINIKSOLVERPOS_NR_JL2_H
...
namespace KDL {
     ...
     */
    class ChainIkSolverPos_NR_JL2// : public ChainIkSolverPos
    {
    public:
        /**
         ...
         * @return
         */
        ChainIkSolverPos_NR_JL2(const Chain& chain,const JntArray& q_min, const JntArray& q_max, ChainFkSolverPos& fksolver,ChainIkSolverVel& iksolver,unsigned int maxiter=100,double eps=1e-6);
        ~ChainIkSolverPos_NR_JL2();

	// mode = 0 - normal operation - find closest
	// mode = 1 - ignore rotation, focus only on translation
	// mode = 2 - focus on Z rotation and translation
	// mode = 3 - steering focused
	// mode = 4 - steering focused + minimize joint delta v

        double CartToJnt(const JntArray& q_init, const Frame& p_in, JntArray& q_out, int mode=0);
        double CartToJntWrist(const JntArray& q_init, const Frame& p_in, JntArray& q_out, int mode=0);
	double myscore(Frame f1, Frame f2, int mode);
	float myscore4(const JntArray& saved_q_init, const JntArray& q_init, const JntArray& q_out);
    private:
        const Chain chain;
        JntArray q_min;
        JntArray q_max;
